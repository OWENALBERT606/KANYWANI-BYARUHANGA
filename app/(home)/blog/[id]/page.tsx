...
import { getDashboardBlogs } from "@/actions/blogs"
import { getServerSession } from "next-auth"
import { authOptions } from "@/config/auth"
import CommentForm from "@/components/Forms/comment-form"
import { getCommentsByBlogId } from "@/actions/comments"
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-6 text-muted-foreground mb-6">
            <div className="flex items-center gap-2">
              <User className="w-5 h-5" />
              <span className="font-medium">By Byaruhanga John Baptist</span>
            </div>
            <div className="flex items-center gap-2">
              <Clock className="w-5 h-5" />
            </div>
        ...
        {post.videoUrl && (
          <div className="mb-12">
            <h2 className="text-xl font-bold mb-2">Video Attachment</h2>
            <video
              controls
        ...
        {post.pdfUrl && (
          <div className="mb-12">
            <h2 className="text-xl font-bold mb-2">PDF Attachment</h2>
            <iframe
              src={post.pdfUrl}
        ...
        <div className="shadow-sm border-top border-black md:ml-24">
          ...
          {comments && comments.length > 0 ? (
            comments.map((comment: any) => (
              <div
                key={comment.id}
                className="mb-6 border-b border-muted pb-4"
              >
                <p>{comment.content}</p>
                <p>BY: {comment?.user.name}</p>
              </div>
